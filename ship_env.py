...
class ShipEnv(Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=np.array([0, -np.pi / 2, 0, -4, -0.1]), high=np.array([150, np.pi / 2, 4.0, 4.0, 0.1]))
    ...
    def calculate_reward(self, obs):
        ...
        if self.last_pos[0] > 5000:
            print("\n Got there")
        return  vx * 1 - vx*d/300 - vy

    def end(self, state_prime, obs):
        if not self.observation_space.contains(obs) or -1 > state_prime[0] or state_prime[0] > 5000 or 160 < state_prime[1] or state_prime[1]< -160:
